...
| Orphan classes | A class in the markup with no rule anywhere |
...
console error, and the gate passed. The undefined-class and invisible-icon checks both exist because of
escapes like that: a class used seven times but never defined left a row of
arrows stacked on top of their labels, silently. When something slips through, add a check for it. That is how the
